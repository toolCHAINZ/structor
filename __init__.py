...
def do_work(bv, instr):
    ...
    if choices is []:
        interaction.show_message_box(
            "Error!", "No variables found. If there's obviously a variable this is a bug.")
        return
    index = interaction.get_choice_input(
        'Variable Selection', 'Please select a variable', choices)
    if index == None:
        return
    ...
    if ssa_var == None:
        ...
        interaction.show_message_box(
            "Error!", "Couldn't find SAA form of variable {}".format(variable))
        return
    mlil_func = instr.function
    result = Structor(ssa_var, mlil_func)
    if len(result.access_info) == 0:
        interaction.show_message_box(
            "Nothing to do", "Can't figure out what to do with this...")
        return

    name = interaction.get_text_line_input(
        "Struct Name", "Choose a name for the struct")
    if name == b"":
    ...
    for ssa_var in result.variables:
        instr.function.source_function.create_user_var(
            ssa_var.var, struct_ptr, ssa_var.var.name)
...
PluginCommand.register_for_medium_level_il_instruction(
    "Structor\\Create Auto Struct", "structor", create_struct)
